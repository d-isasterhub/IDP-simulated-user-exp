...
get_agreement_dt <- function(questions, dt_h, dt_llm) {
...
get_barplot <- function(dt, column, title) {
  dt %>% ggplot(aes(as.factor(get(column)))) +
    geom_bar() +
    xlab(column) +
    ggtitle(title)
}
...
human_dt <- get_human_dt()
llm_dt <- get_llm_dt(path)
agreement_dt <- get_agreement_dt(questions, human_dt, llm_dt)
...
prompting <- paste(
  "Prompt",
  accuracy,
  "accuracy of classification guesses."
)
...
get_agreement_scatterplot(agreement_dt, title)
get_barplot(agreement_dt, "diff", title)
...
get_agreement_scatterplot(mean_agreement_dt, title)
get_barplot(mean_agreement_dt, "diff", title)
```
