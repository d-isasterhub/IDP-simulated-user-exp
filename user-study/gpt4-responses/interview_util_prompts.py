...
OBJECTIVE_DESCRIPTION_3 = "Great, you just found a way to distinguish between Least Auklet and "\
    ...
    "I will show you four 4 groups of images, one for each of the bird species. The top image is "\
    "always the original image and the bottom image displays the image combined with the heatmap "\
    "generated by the machine-learning model. The goal of your task is to understand the model's "\
    "reasoning based on the highlighted features (areas)."
...
CLASS_PREDICTION_Q1 = "Please choose the bird species for this image that the xai model might "\
    "choose based on its reasoning (highlighted area). Choose one of the following options:\n"\
    "- Crested Auklet\n"\
    "- Least Auklet\n"\
    "- Parakeet Auklet\n"\
    "- Rhinoceros Auklet"